    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900/80 border-b border-gray-800 px-4 py-3 sticky top-0 z-10 backdrop-blur">
        <div className="max-w-6xl mx-auto">
          <p className="text-xs text-gray-500 uppercase tracking-widest mb-0.5">{t('bracket.publicTitle')}</p>
      ...
      {byStage.final?.[0]?.winner_id && (
        <div className="text-center pb-8">
          <p className="text-xs text-yellow-500 uppercase tracking-widest mb-1">🏆 {t('bracket.champion')}</p>
      ...
      <div className="text-center pb-6 text-xs text-gray-700">
        Xem trực tiếp · <Link to={`/tournament/${tournamentId}/live`} className="underline text-gray-500">{t('bracket.liveScoreboard')}</Link>
      </div>
    </div>
  )
}
...
function BracketCard({ match, playerMap, t }) {
  ...
  return (
    <div className={cn(
      ...
      live ? 'border-green-500/50 bg-gray-800/80' : done ? 'border-gray-700/50 bg-gray-900/40' : 'border-gray-800 bg-gray-900/60',
    )}>
      {live && (
        <div className="flex items-center gap-1 mb-1.5">
          <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
          <span className="text-[10px] font-bold text-green-400">{t('common.live')}</span>
        </div>
      )}
      ...
      {score && (
        <p className="text-[10px] text-gray-500 mt-1.5 font-mono text-center">{score}</p>
      )}
      {!p1 && !p2 && (
        <p className="text-xs text-gray-600 text-center py-1">{t('bracket.waitingResult')}</p>
      )}
    </div>
  )
}
...
function BracketPlayer({ player, isWinner, isForfeit, tbd }) {
  return (
    <div className={cn('flex items-center justify-between gap-1', isWinner && 'font-bold')}>
      <span className={cn('text-xs truncate', isWinner ? 'text-white' : 'text-gray-400', isForfeit && 'line-through text-gray-600')}>
        {player?.name ?? tbd}
      </span>
